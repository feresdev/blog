...
import FormBlog from "@/components/pages/perfil/FormBlog";
...
export default function Perfil() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const resUsuario = await apiClient("/usuario", {
          method: "GET"
        });
        setDataUsuario(resUsuario)

        const resBlog = await apiClient("/blog/usuario", {
          method: "GET"
        });
        setDataBlog(resBlog)
      } catch (err) {
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="col-span-3">
            <h1 className="text-2xl font-semibold pb-3">Mis blogs</h1>
          </div>
